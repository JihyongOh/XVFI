...
def get_train_data(args, max_t_step_size):
	...
	dataloader = torch.utils.data.DataLoader(data_train, batch_size=args.batch_size,drop_last=True, shuffle=True, num_workers=int(args.num_thrds), pin_memory=False)
	return dataloader
	...
	if args.dataset == 'X4K1000FPS':
		if validation:
			...
			if args.S_tst == 3:
				...
				padding = 400
			frames = np.pad(frames,[[0,0],[0,padding],[0,0],[0,0]])
...
def RGBframes_np2Tensor(imgIn, channel):  
	...
	if channel == 1:
		# rgb --> Y (gray)
		imgIn = np.sum(imgIn * np.reshape([65.481, 128.553, 24.966], [1, 1, 1, 3]) / 255.0, axis=3, keepdims=True) + 16.0
...
def make_2D_dataset_X_Train(dir):
	...
	print("The number of total training samples : {} which has 65 frames each.".format(len(framesPath))) ## 4408 folders which have 65 frames each
	return framesPath
...
class Vimeo_Train(data.Dataset):
	def __init__(self, args):
		...
		self.t = 0.5
		self.framesPath = []
		f = open(os.path.join(args.vimeo_data_path,'tri_trainlist.txt'), 'r') # '../Datasets/vimeo_triplet/sequences/tri_trainlist.txt'
		while True:
			scene_path = f.readline().split('\n')[0]
			if not scene_path: break
			frames_list = sorted(glob.glob(os.path.join(args.vimeo_data_path,'sequences/',scene_path,'*.png'))) # '../Datasets/vimeo_triplet/sequences/%05d/%04d/*.png'
...
def metrics_evaluation_X_Test(pred_save_path, test_data_path, metrics_types, flow_flag=False, multiple=8, server=None):
	...
	for scene_idx, scene_folder in enumerate(pred_framesPath):
		...
		for frameIndex, pred_frame in enumerate(pred_candidate):
			...
			if frameIndex > 0 and frameIndex < multiple:
				...
				target_img = cv2.imread(pred_frame.replace(pred_save_path, test_data_path)).astype(np.float32)  # BGR, [0,255]
				pred_frame_split = pred_frame.split('/')
				msg = "[x%d] frame %s, " % (multiple, os.path.join(pred_frame_split[-3],pred_frame_split[-2],pred_frame_split[-1])) # per frame
				...
				if "tOF" in keys:  # tOF
					...
					if frameIndex == 1:
						...
						pre_out_grey = cv2.cvtColor(cv2.imread(pred_candidate[0]).astype(np.float32), cv2.COLOR_BGR2GRAY)  #### CAUTION BRG
	...
	for key in keys:
		Total_avg_dict["TotalAvg_" + key] = pd.Series(np.float32(Total_avg_dict["TotalAvg_" + key])) # replace key (update)
...
def get_batch_images(args, save_img_num, save_images): ## For visualization during training phase
	...
	for b in range(save_img_num):
		...
		log_img[(b) * args.patch_size:(b + 1) * args.patch_size, 0 * args.patch_size:1 * args.patch_size,:] = simple_mean_img_tmp
		log_img[(b) * args.patch_size:(b + 1) * args.patch_size, 1 * args.patch_size:2 * args.patch_size,:] = output_img_tmp
		log_img[(b) * args.patch_size:(b + 1) * args.patch_size, 2 * args.patch_size:3 * args.patch_size,:] = gt_img_tmp
		log_img[(b) * args.patch_size:(b + 1) * args.patch_size, 3 * args.patch_size:4 * args.patch_size,:] = output_coarse_flow_tmp
		log_img[(b) * args.patch_size:(b + 1) * args.patch_size, 4 * args.patch_size:5 * args.patch_size,:] = output_fine_flow_tmp
		log_img[(b) * args.patch_size:(b + 1) * args.patch_size, 5 * args.patch_size:6 * args.patch_size,:] = occ_map_tmp
